...
NOAA_Str1<- str_split(Main_df$NOAA_ID, pattern = ",")
#NOAA_Str1 <- str_split(str_sub(NOAA_Str), pattern = ",")
NOAA_Str <- paste0(NOAA_Str1, "_tr", ".csv")
NOAA_Str_compare <- paste0(NOAA_Str1, ".csv")
NOAA_Str <- as.list(NOAA_Str)
NOAA_Str_compare <- as.list(NOAA_Str_compare)
...
for (i in 1:length(files)){
  ...
  print(fp) 
  if (files[i] %in% NOAA_Str_compare == TRUE){
