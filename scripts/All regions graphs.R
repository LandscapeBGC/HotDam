...
region_list <- list("Midwest", "Northeast", "Northern Rockies", "Northwest", "Ohio Valley", "South", "Southeast", "Southwest", "West")

#empty list for climate region
region_whole <- vector('list')
i <- 1
...
df2 <- map_df(region_whole, ~read.csv(.), .id = "origin")
regions_plot <- df2 %>% ggplot() +
  aes(df2$x, df2$y) +
  geom_point() +
  geom_smooth() +
  labs(title = paste0("regions")) +
  theme(plot.title = element_text(size = 24)) +
  theme(plot.title = element_text(hjust = .5))+
  theme(axis.text = element_text(size = 20))+
  #facet_wrap(~ df2$origin ,scales = "fixed")
setwd("~/HotDam3/results/figures")
ggsave("regions_plot.png", device = "png")
